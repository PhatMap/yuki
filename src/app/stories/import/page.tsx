...
export default function ImportNovelPage() {
  ...
  const totalWordCount = useMemo(
    () => detectedChapters.reduce((total, chapter) => total + chapter.wordCount, 0),
    [detectedChapters],
  );
  ...
  const importStepIndex = !hasText ? 0 : hasDetectedChapters ? 2 : 1;
  ...
  const nextAction = !hasText
    ? { title: "Chọn file TXT", description: "Nạp toàn bộ truyện trong một file." }
    : !hasDetectedChapters
      ? { title: "Tách chương", description: "Yuki sẽ nhận diện chương để bạn kiểm tra." }
      : {
          title: "Lưu và phân tích",
          description: "Kiểm tra chương đã tách, rồi lưu truyện để chạy phân tích.",
        };

  const importSteps = [
    {
      label: "Chọn file TXT",
      status: importFileName || "Chưa chọn file",
    },
    {
      label: "Tách chương",
      status: hasDetectedChapters
        ? `${detectedChapters.length.toLocaleString("vi-VN")} chương`
        : hasText
          ? "Sẵn sàng"
          : "Chưa có file",
    },
    {
      label: "Lưu và phân tích",
      status: hasDetectedChapters ? "Sẵn sàng lưu" : "Chưa sẵn sàng",
    },
  ];
  ...
  async function handleImportTextFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.name.toLowerCase().endsWith(".txt")) {
      setCreateError("Yuki hiện chỉ hỗ trợ file .txt ở bước này.");
      event.target.value = "";
      return;
    }
  ...
  function handleCancelImportProcessing() {
    ...
    setImportProgress((current) =>
      current
        ? { ...current, status: "completed", message: "Đã hủy xử lý nhập truyện." }
        : undefined,
    );
  }
  ...
  async function handleCreateStoryFromImport() {
    ...
    try {
      ...
      const newStory: Story = {
        ...
        description:
          processedImport.chapters[0]?.cleanContent.slice(0, 240) ||
          "Truyện dài đã nhập.",
        author: author.trim(),
        genre: "adventure",
        tone: "dramatic",
      ...
      await saveImportedStoryData({
        story: newStory,
        chapters: processedImport.chapters,
        chunks: processedImport.chunks,
        analysisStatus: processedImport.analysisStatus,
      });
    } catch (error) {
      if (error instanceof DOMException && error.name === "AbortError") {
        setCreateError("");
    ...
    <PageShell>
      <PageContainer className="max-w-7xl">
        <PageHeader
          ...
          description="Chọn file TXT, tách chương, kiểm tra kết quả rồi lưu để phân tích."
          action={
            <>
              <Button asChild variant="outline" size="sm">
                <Link href="/">Về Dashboard</Link>
        ...
        {isProcessingImport ? (
          <JobStatusCard
            ...
            description="Giữ trang này mở cho đến khi hoàn tất."
        ...
        <div className="grid gap-6 xl:grid-cols-[1fr_420px]">
          ...
          >
            <div ref={uploadSectionRef} className="rounded-xl border border-dashed bg-background p-4">
              <p className="text-sm font-medium">File TXT chứa toàn bộ truyện</p>
              <p className="mt-1 text-xs text-muted-foreground">
                Yuki tự nhận diện chương từ tiêu đề trong file.
              </p>
              <Input
                ref={fileInputRef}
                className="mt-3"
          ...
          <div className="space-y-6">
            <SectionCard
              ...
              description="Tùy chọn. Yuki tự lấy tên từ file nếu bạn để trống."
            >
              ...
              <div ref={preflightSectionRef}>
                {hasDetectedChapters ? (
                  <PreflightSummary
                    ...
                    chunkCount={detectedChunks.length}
                    firstChapterNumber={detectedChapters[0]?.chapterNumber}
                    lastChapterNumber={detectedChapters.at(-1)?.chapterNumber}
                    showSingleChapterWarning={detectedChapters.length === 1 && totalWordCount >= 5000}
                    duplicateChapterNumbers={duplicateChapterNumbers}
                    genericTitleCount={genericTitleCount}
                  />
                ) : (
                  <EmptyState
                    title="Chưa có dữ liệu tách chương"
                    description="Chọn file TXT, sau đó bấm Tách chương."
                  />
                )}
              </div>
            </SectionCard>
              ...
              <div className="grid gap-4 xl:grid-cols-[420px_minmax(0,1fr)]">
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Hiển thị {visibleChapters.length.toLocaleString("vi-VN")} / {filteredChapters.length.toLocaleString("vi-VN")} chương
                  </p>
                  ...
                  <div className="max-h-[520px] space-y-2 overflow-auto pr-1">
                    {visibleChapters.map((chapter) => (
                      ...
                      >
                        <p className="text-xs text-muted-foreground">Chương {chapter.chapterNumber}</p>
                        <p className="mt-1 line-clamp-2 text-sm font-medium">
                          {chapter.title || "(Không có tiêu đề)"}
                        </p>
                        <p className="mt-1 text-xs text-muted-foreground">
                          {chapter.wordCount.toLocaleString("vi-VN")} từ · {(chunkCountsByChapterId[chapter.id] ?? 0).toLocaleString("vi-VN")} đoạn
                        </p>
                      </button>
                    ))}
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  {selectedChapter ? (
                    <div className="rounded-xl border bg-background p-4">
                      <p className="text-xs text-muted-foreground">Chương {selectedChapter.chapterNumber}</p>
                      <p className="mt-1 text-sm font-medium">
                        {selectedChapter.title || "(Không có tiêu đề)"}
                      </p>
                      <p className="mt-1 text-xs text-muted-foreground">
                        {selectedChapter.wordCount.toLocaleString("vi-VN")} từ · {(chunkCountsByChapterId[selectedChapter.id] ?? 0).toLocaleString("vi-VN")} đoạn
                      </p>
...
function PreflightSummary({
  ...
  showSingleChapterWarning: boolean;
  ...
  return (
    <div className="space-y-3">
      <div className="grid gap-2 text-sm md:grid-cols-2">
        <PreflightRow label="Số chương" value={chapterCount.toLocaleString("vi-VN")} />
        <PreflightRow label="Số từ ước tính" value={wordCount.toLocaleString("vi-VN")} />
        <PreflightRow label="Số đoạn" value={chunkCount.toLocaleString("vi-VN")} />
        <PreflightRow label="Chương đầu" value={`Chương ${firstChapterNumber ?? "-"}`} />
        <PreflightRow label="Chương cuối" value={`Chương ${lastChapterNumber ?? "-"}`} />
      </div>
      {showSingleChapterWarning ? (
        <WarningText text="Có thể Yuki chưa nhận diện đúng tiêu đề chương." />
      ) : null}
      {duplicateChapterNumbers.length > 0 ? (
        <WarningText text={`Phát hiện chapter number trùng: ${duplicateChapterNumbers.join(", ")}.`} />
      ) : null}
      {genericTitleCount > 0 ? (
        <WarningText text={`Có ${genericTitleCount.toLocaleString("vi-VN")} chương có title trống hoặc quá chung.`} />
      ) : null}
    </div>
  );
}
